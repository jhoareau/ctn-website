...
export class NewsForm extends React.Component {
  ...
  populate(route) {
    Request.get(route).end((err, data) => {
      data = data.body;
      this.setState({newsTitle: data.title, newsLink: data.href, newsDescription: data.text});
      ...
      <div>
        ...
        <form className="form-horizontal mdl-shadow--2dp" onSubmit={this.saveUpload}>
          ...
          <fieldset className="form-group mdl-textfield mdl-js-textfield mdl-textfield--floating-label">
            ...
            <textarea ref="newsDesc" id="newsDesc" className="mdl-textfield__input" value={this.state.newsDescription} onChange={this.updateNewsDescription} />
...
class NewsItem extends React.Component {
  ...
  render() {
    return (
      <div className="gestion_news">
        <CustomLink href={'/news/update/' + this.props._id} root={true}>Editer la news: {this.props.title}</CustomLink> <a href='#' className="delete_news" onClick={this.confirmDelete}>Supprimer</a>
      </div>
    );
  }
}
...
export class NewsAdmin extends React.Component {
  ...
  refresh() {
    this.populate(this.props.route);
  }

  render() {
    return (
      <div className="news_container">
        {this.state.newsList.map((el, i) => <NewsItem {...el} key={'item'+i} eventHandler={this.refresh} />)}
      </div>
    );
  }
}
NewsAdmin.defaultProps = {
  newsList: []
}
